...
function getViolinWTracksSongs() {
  // Violin w/ Tracks Cocktail Hour songs
  const songs = [
  ...
  console.log(`✅ Loaded ${songs.length} Violin w/ Tracks Cocktail Hour songs`);
  return songs;
}
...
async function findOrCreateSong(songTitle, artistName) {
  ...
  const newSong = {
    ...
    ensembles: ["Violin w/ Tracks (Violin + DJ)"],
    genres: [],
    specialMomentTypes: [],
    thcPercent: null,
    notes: "",
...
async function updateSongForViolinWTracks(song) {
  ...
  const ensemble = "Violin w/ Tracks (Violin + DJ)";
...
async function main() {
  try {
    console.log('🎻 Starting Violin w/ Tracks Cocktail Hour songs import...\n');
    
    const violinWTracksSongs = getViolinWTracksSongs();
    
    if (violinWTracksSongs.length === 0) {
      console.log('⚠️  No songs found');
      process.exit(0);
    }
    
    console.log(`\n📝 Processing ${violinWTracksSongs.length} songs...\n`);
    ...
    for (const songData of violinWTracksSongs) {
      try {
        ...
        const needsUpdate = 
          ...
          !song.ensembles || !Array.isArray(song.ensembles) || !song.ensembles.includes('Violin w/ Tracks (Violin + DJ)');
        
        if (needsUpdate) {
          await updateSongForViolinWTracks(song);
          updated++;
        } else {
