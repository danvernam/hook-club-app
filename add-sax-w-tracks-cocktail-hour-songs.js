...
function getSaxWTracksSongs() {
  // Sax w/ Tracks Cocktail Hour songs
  const songs = [
  ...
  console.log(`✅ Loaded ${songs.length} Sax w/ Tracks Cocktail Hour songs`);
  return songs;
}
...
async function updateSongForSaxWTracks(song) {
  ...
  const ensemble = "Sax w/ Tracks (Sax + DJ)";
...
async function main() {
  try {
    console.log('🎷 Starting Sax w/ Tracks Cocktail Hour songs import...\n');
    
    const saxWTracksSongs = getSaxWTracksSongs();
    
    if (saxWTracksSongs.length === 0) {
      console.log('⚠️  No songs found');
      process.exit(0);
    }
    
    console.log(`\n📝 Processing ${saxWTracksSongs.length} songs...\n`);
    ...
    for (const songData of saxWTracksSongs) {
      try {
        ...
        const needsUpdate = 
          ...
          !song.ensembles || !Array.isArray(song.ensembles) || !song.ensembles.includes('Sax w/ Tracks (Sax + DJ)');
        
        if (needsUpdate) {
          await updateSongForSaxWTracks(song);
          updated++;
        } else {
